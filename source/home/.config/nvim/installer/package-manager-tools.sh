...
function aur_install()
{
    ...
    if yes_no_prompt "Install '$package' from the AUR in build dir '$build_dir'?"; then
        ...
        print_note "If you wish to update this paclage, 'cd' into '$build_dir/$package', run 'git pull', and then '$AUR_INSTALL'."
...
function install_packages()
{
    ...
    case $PackageManager in
        "yay")
            verbose_run xargs -o $PackageManager -S --color=always <<< $packages
            ;;
        "pacman")
            verbose_run sudo xargs -o $PackageManager -S --color=always <<< $packages
            ;;
        "apt"|"apt-get")
